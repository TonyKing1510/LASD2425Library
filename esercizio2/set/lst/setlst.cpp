...
SetLst<Data>::SetLst(const TraversableContainer<Data>& q) {
  q.Traverse([this](const Data& dat) {
    Insert(dat); // Usa il metodo Insert per inserire ogni elemento
  });
}
...
SetLst<Data>::SetLst(MappableContainer<Data>&& q) {
  q.Map([this](Data& dat) {
    Insert(std::move(dat)); // Usa il metodo Insert per inserire ogni elemento
  });
}

template <typename Data>
SetLst<Data>::SetLst(const SetLst<Data>& other) : List<Data>(other) {
  // Copia ogni elemento da 'other' in 'this'
  other.Traverse([this](const Data& dat) {
    Insert(dat);  // Usa Insert per mantenere la proprietà di unicità e ordine
  });
}
...
SetLst<Data>::SetLst(SetLst<Data>&& other) noexcept
  ...
  // Non serve altro: la classe base gestisce lo spostamento
  }
...
Data SetLst<Data>::PredecessorNRemove(const Data& d) {
  ...
  // Serve per trovare il nodo *prima* del predecessore (per scollegarlo)
  if (pred == this->head) {
    this->head = pred->ptrnext;
  } else {
    // Scorri per trovare chi punta a pred
    toRemove = this->head;
...
void SetLst<Data>::RemovePredecessor(const Data& d) {
  ...
  // Scorri finché trovi un nodo maggiore o uguale a d
...
typename List<Data>::Node* SetLst<Data>::BinarySearch(const Data& val, typename List<Data>::Node*& pred) const noexcept {
  ...
  while (left <= right) {
    ...
    if (midNode->Element == val) {
      pred = prev;          // utile se vuoi sapere il precedente anche se trovato
      return midNode;
